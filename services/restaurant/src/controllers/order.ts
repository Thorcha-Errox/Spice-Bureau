...
export const createOrder = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const { paymentMethod, addressId, note } = req.body;
  ...
  const riderAmount = distance < 1 ? 10 : Math.ceil(distance) * 17;
...
export const getRiderStats = TryCatch(async (req, res) => {
  if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
    return res.status(403).json({
      message: "Forbidden",
    });
...
export const fetchRiderOrders = TryCatch(async (req, res) => {
  if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
    return res.status(403).json({
      message: "Forbidden",
    });
